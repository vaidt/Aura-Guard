...
| `INV-XIM-01` | `REQ-XIM-01` | cross-implementation | Node CLI spawns the Python reference verifier, compares per-test status for every common check id (excluding `impl:cross-implementation` itself), and emits PASS iff all match. Browser and `--no-cross-impl` report `NOT APPLICABLE`. | `impl:cross-implementation` | `checkCrossImplementation` | IMPLEMENTED |
